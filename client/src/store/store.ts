...
export const store = configureStore({
  reducer: {
    ...
    [authApi.reducerPath]: authApi.reducer, // Добавляем authApi в редюсеры

    auth: authReducer,
  },
  middleware: (getDefaultMiddleware) =>
    getDefaultMiddleware().concat(sshApi.middleware, authApi.middleware), // Добавляем middleware для обоих API
});
