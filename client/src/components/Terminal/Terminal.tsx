...
const XTermConsole: React.FC<XTermConsoleProps> = ({ sessionId }) => {
  ...
  const [isMounted, setIsMounted] = useState(false); // Флаг для отслеживания инициализации
  const inputCursorRef = useRef<number>(0); // Позиция курсора
  const promptLineRef = useRef<string>(""); // Строка последнего приглашения
  ...
  useLayoutEffect(() => {
    if (terminalRef.current) {
      setIsMounted(true); // Устанавливаем, что компонент полностью инициализирован
    }
  }, []);
  ...
  useEffect(() => {
    ...
    // Подключение WebSocket только после инициализации терминала
  ...
  }, [isMounted]); // Зависимость от isMounted
